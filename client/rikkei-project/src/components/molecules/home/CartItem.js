...
const CartItem = ({ handleDeleteCart, cartgame }) => {
  ...
  return (
    <div className='box' >
      <div style={{ position: "relative" }} onClick={HandleProductCart}>
          ...
          <li style={{ display: "flex", height: "30px", textAlign: "center" }} >
            {
              cartgame?.length ?
                <Link to='/product' target='_blank'>
                  <p className='productseemore' style={{ marginLeft: "20px" }}> Xem tất cả {cartgame?.length} sản phẩm</p>
                </Link> :
